			...
			try {
				...
				const sub = subscribe({
					next: async (evt: any) => {
                        ...
                        // Metadata handling: fetch from Lambda endpoint (bypasses AppSync permissions)
                        if (m?.senderId === 'assistant-bot' && (!m.metadata || m.metadata === 'null')) {
                            const msgId = m.id;
                            console.log('[metadata] Scheduling fetch for', msgId.slice(0, 8), 'in 1.5s');
                            setTimeout(async () => {
                                try {
                                    const { getFlags } = await import('../utils/flags');
                                    const { ASSISTANT_ENDPOINT } = getFlags();
                                    const endpoint = String(ASSISTANT_ENDPOINT || '').replace(/\/$/, '');
                                    if (!endpoint) {
                                        console.warn('[metadata] No ASSISTANT_ENDPOINT configured');
                                        return;
                                    }
                                    
                                    const url = `${endpoint}/agent/weekend-plan`;
                                    console.log('[metadata] Fetching via POST for:', msgId.slice(0, 8));
                                    const response = await fetch(url, {
                                        ...
                                        body: JSON.stringify({
                                            fetchMetadata: true,
                                            messageId: msgId,
                                            userId: m.conversationId.split('::')[1]
                                        })
                                    });
                                    
                                    if (!response.ok) {
                                        console.warn('[metadata] Fetch failed:', response.status, response.statusText);
                                        return;
                                    }
                                    
                                    const data = await response.json();
                                    console.log('[metadata] Response:', { hasMetadata: !!data?.metadata, metadata: data?.metadata ? String(data.metadata).slice(0, 100) : null });
                                    
                                    if (data?.metadata && data.metadata !== 'null') {
                                        console.log('[metadata] ✅ Fetched successfully for', msgId.slice(0, 8));
                                        setMessages(prev => mergeDedupSort(prev, [{ ...m, metadata: data.metadata }]));
                                    } else {
                                        console.warn('[metadata] ⚠️ No metadata in response');
                                    }
                                } catch (e) {
                                    console.warn('[metadata] ❌ Failed to fetch:', e);
                                }
                            }, 1500); // Wait 1.5s for Lambda to finish + DynamoDB replication
                        }
				...
				// Subscribe to message updates (for metadata changes from Lambda)
				console.log('[sub:update] Starting subscription setup for cid:', cid);
				try {
					console.log('[sub:update] Importing subscribeMessageUpdates...');
					const { subscribeMessageUpdates } = await import('../graphql/messages');
					console.log('[sub:update] Import successful, calling subscribeMessageUpdates...');
					const subscribeUpdates = subscribeMessageUpdates(cid);
					console.log('[sub:update] Subscribing to updates...');
					const updateSub = subscribeUpdates({
						next: async (evt: any) => {
							console.log('[sub:update] Received event:', evt);
							try { const { DEBUG_LOGS } = getFlags(); if (DEBUG_LOGS) console.log('[sub:update] evt', evt); } catch {}
							const m = evt?.data?.onUpdateMessage;
							if (!m || !m.id) return;
							
							// Log metadata updates for assistant messages
							if (m?.senderId === 'assistant-bot') {
								if (m.metadata && m.metadata !== 'null') {
									console.log('[metadata] ✅ Received metadata via UPDATE subscription for', m.id.slice(0, 8));
									console.log('[metadata] Metadata content:', String(m.metadata).slice(0, 100));
								}
							}
							
							// Update the message in state
							setMessages(prev => mergeDedupSort(prev, [m]));
						},
						error: (err: any) => {
							console.warn('[sub:update] Subscription error:', err);
						},
					});
					updateSubRef.current = updateSub;
					console.log('[sub:update] ✅ Successfully subscribed to message updates');
				} catch (e) {
					console.error('[sub:update] ❌ Failed to subscribe:', e);
				}
				...
				// Debug: Check metadata for assistant messages
				try {
					const assistantMsgs = decorated.filter((m: any) => m.senderId === 'assistant-bot');
					if (assistantMsgs.length > 0) {
						const latest = assistantMsgs[0];
						console.log('[debug] Latest assistant message:', {
							id: latest.id?.slice(0, 8),
							hasMetadata: !!latest.metadata,
							metadata: latest.metadata ? String(latest.metadata).slice(0, 100) : null
						});
					}
				} catch {}
				setMessages(prev => {
